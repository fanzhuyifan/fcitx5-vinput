    ...
    try {
      ...
      if (postprocessing_action == PostprocessingState::CommitRaw) {
        const auto raw = std::find_if(
            pipeline_result.payload.candidates.begin(), pipeline_result.payload.candidates.end(),
            [](const auto& candidate) { return candidate.source == vinput::result::kSourceRaw; });
        if (raw == pipeline_result.payload.candidates.end()) {
          pipeline_result.payload = {};
        } else {
          pipeline_result.payload.commitText = raw->text;
          pipeline_result.payload.candidates = {*raw};
        }
