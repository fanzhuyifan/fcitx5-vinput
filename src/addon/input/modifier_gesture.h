...
public:
  ...
  enum class Action : std::uint8_t { Dictation, Command, Palette };
  enum class Mode : std::uint8_t { Tap, Hold, Both };
  enum class Event : std::uint8_t { None, Tap, HoldStart, HoldRelease, HoldCancel };
  struct Binding {
    fcitx::Key key;
    Action action;
    Mode mode;
...
private:
  ...
  [[nodiscard]] bool matches(const Binding& binding, fcitx::KeyStates down) const;
  void clearGesture();
