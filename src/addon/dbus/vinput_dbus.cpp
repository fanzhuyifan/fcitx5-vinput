...
void VinputEngine::callCancelOperation(bool commit_raw_text) {
  if ((bus_ == nullptr) || pending_cancel_call_slot_) {
    return;
  }
...
void VinputEngine::ensureStatusSync() {
  const bool needs_sync = recording_cancel_requested_ || session_ || (status_ic_ != nullptr);
  if (!needs_sync) {
...
void VinputEngine::stopStatusSyncIfIdle() {
  if (status_sync_event_ && !recording_cancel_requested_ && !session_ && (status_ic_ == nullptr)) {
